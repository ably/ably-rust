...
async fn rsa3_bearer_auth_with_explicit_token() -> Result<()> {
    let mock =
        MockHttpClient::with_handler(|_req| MockResponse::json(200, &json!({"channelId": "test"})));
...
async fn rsa6b_explicit_capability_preserved() {
    ...
    assert_eq!(
        req.capability.as_deref(),
        Some(r#"{"channel-a":["publish","subscribe"]}"#)
    );
}
...
async fn rsan1c4_idempotent_id_generated() -> Result<()> {
    ...
    assert!(
        parts[0].len() >= 12,
        "Base64 part should be at least 12 chars"
    );
    assert_eq!(parts[1], "0");
    Ok(())
}
...
async fn rsan3c_get_returns_paginated_annotations() -> Result<()> {
    ...
    let ann1 = &items[0];
    assert_eq!(
        ann1.annotation_type.as_deref(),
        Some("com.example.reaction")
    );
...
async fn rsa4e_rest_callback_error_produces_40170() {
    ...
    let result: Result<crate::http::PaginatedResult<crate::rest::Message>> =
...
async fn rsa16c_updated_after_request_token() {
    ...
    let td = client.auth().request_token(None, None).await.unwrap();
...
async fn rsa12a_client_id_passed_to_callback() -> Result<()> {
    ...
    impl AuthCallback for RecordingCb {
        ...
        ) -> std::pin::Pin<Box<dyn Send + futures::Future<Output = Result<AuthToken>> + 'a>>
        {
            ...
            Box::pin(async { Ok(AuthToken::Details(TokenDetails::token("cb-token".into()))) })
        }
    }

    let received = Arc::new(StdMutex::new(Vec::new()));
    let cb = Arc::new(RecordingCb {
...
async fn rsa8d_auth_callback_invoked() -> Result<()> {
    ...
    impl AuthCallback for FlagCb {
        ...
        ) -> std::pin::Pin<Box<dyn Send + futures::Future<Output = Result<AuthToken>> + 'a>>
        {
            ...
            Box::pin(async {
                Ok(AuthToken::Details(TokenDetails::token(
                    "callback-token".into(),
                )))
            })
        }
    ...
    let mock = MockHttpClient::with_handler(|_req| MockResponse::json(200, &json!({})));
    let client = ClientOptions::with_auth_callback(Arc::new(FlagCb {
...
async fn rsa8c_auth_url_invoked() -> Result<()> {
    ...
    let client =
        ClientOptions::with_auth_url("https://auth.example.com/token").rest_with_mock(mock)?;
...
async fn rsa8c_auth_url_query_params() -> Result<()> {
    ...
    client.request("GET", "/channels/test").send().await?;
    let reqs = get_mock(&client).captured_requests();
    let query: std::collections::HashMap<String, String> = reqs[0]
        .url
        .query_pairs()
...
async fn rsa8c_auth_url_returns_jwt() -> Result<()> {
    ...
    let client =
        ClientOptions::with_auth_url("https://auth.example.com/jwt").rest_with_mock(mock)?;
...
async fn rsa4b1_preemptive_renewal() -> Result<()> {
    ...
    let mock = MockHttpClient::with_handler(|_req| MockResponse::json(200, &json!([])));
    let client = ClientOptions::with_auth_callback(Arc::new(ExpiringCb {
        count: count.clone(),
    }))
    .rest_with_mock(mock)?;
    ...
    assert_eq!(
        history_reqs.len(),
        1,
        "no failing request with the expired token"
    );
    let auth = history_reqs[0]
        .headers
        .iter()
        .find(|(k, _)| k == "authorization")
        .map(|(_, v)| v.as_str())
...
async fn rsa1_token_auth_takes_precedence() -> Result<()> {
    use crate::auth::{AuthCallback, AuthOptions, AuthToken, TokenDetails, TokenParams};
    ...
    let mock = MockHttpClient::with_handler(|_req| MockResponse::json(200, &json!({})));
    let client = ClientOptions::new("appId.keyId:keySecret").rest_with_mock(mock)?;
    ...
    let mock = MockHttpClient::with_handler(|req| {
        if let Some(body) = &req.body {
            let body: serde_json::Value = rmp_serde::from_slice(body)
                ...
                .unwrap();
            assert!(
                body.get("issuedBefore").is_none(),
                "issuedBefore should be omitted"
            );
        }
        MockResponse::json(
            200,
...
async fn rsa5c_ttl_from_default_token_params() {
    ...
    let req = client
        .auth()
        .create_token_request(None, None)
        .await
        .unwrap();
...
async fn rsa5d_explicit_ttl_overrides_default() {
    ...
    let req = client
        .auth()
        .create_token_request(Some(&params), None)
        .await
        .unwrap();
...
async fn rsa10b_authorize_with_explicit_token_params() -> Result<()> {
    ...
    let cb = Arc::new(ParamCapture {
        params: Mutex::new(Vec::new()),
    });
    let mock = MockHttpClient::with_handler(|_req| MockResponse::json(200, &json!({})));
    let client = ClientOptions::with_auth_callback(cb.clone()).rest_with_mock(mock)?;
...
async fn rsa10h_authorize_with_auth_options() -> Result<()> {
    use crate::auth::{AuthCallback, AuthOptions, AuthToken, Credential, TokenParams};
    use std::sync::{
    ...
    let new_cb = Arc::new(FlagCallback {
        called: AtomicBool::new(false),
    });
    let mock = MockHttpClient::with_handler(|_req| MockResponse::json(200, &json!({})));
    let client = ClientOptions::with_auth_callback(new_cb.clone()).rest_with_mock(mock)?;
...
async fn rsa10j_authorize_when_already_authorized() -> Result<()> {
    ...
    let mock = MockHttpClient::with_handler(|_req| MockResponse::json(200, &json!({})));
    let client = ClientOptions::with_auth_callback(cb).rest_with_mock(mock)?;
...
async fn rsa10k_authorize_with_query_time() -> Result<()> {
    ...
    assert!(
        time_requested.load(Ordering::SeqCst),
        "authorize with queryTime should request /time"
    );
    Ok(())
}
...
async fn rsa4_auth_callback_triggers_token_auth() -> Result<()> {
    ...
    assert!(
        ...
        "Expected Bearer auth from callback"
    );
    Ok(())
}
...
async fn rsa4b_token_renewal_limit() -> Result<()> {
    ...
    assert_eq!(
        count, 4,
        "exactly one renewal cycle, got {} requests",
        count
    );
    Ok(())
}
...
async fn rsa4c3_auth_callback_error_while_connected() -> Result<()> {
    ...
    assert_eq!(
        err.code,
        Some(crate::error::ErrorCode::ErrorFromClientTokenCallback.code())
    );
    Ok(())
}
...
async fn rsa7b_client_id_from_auth_callback_token_details() -> Result<()> {
    ...
    let td = client
        .auth()
        .token_details()
        .expect("should have token details");
    assert_eq!(
...
async fn rsa8_token_auth_with_native_token() -> Result<()> {
    ...
    let mock =
        MockHttpClient::with_handler(|_req| MockResponse::json(200, &json!([1234567890000_i64])));
    let client = ClientOptions::with_token("native-ably-token".to_string()).rest_with_mock(mock)?;
...
async fn rsa8_token_auth_with_jwt() -> Result<()> {
    ...
    let mock =
        MockHttpClient::with_handler(|_req| MockResponse::json(200, &json!([1234567890000_i64])));
    let client = ClientOptions::with_token(jwt.clone()).rest_with_mock(mock)?;
...
async fn rsa8d_auth_callback_error_propagated() -> Result<()> {
    ...
    let err = client
        ...
        .send()
        .await
        .expect_err("Should propagate callback error");
    assert_eq!(
        err.code,
        Some(crate::error::ErrorCode::ErrorFromClientTokenCallback.code())
    );
    assert!(err
        ...
        .as_deref()
        .unwrap()
        .contains("callback deliberately failed"));
    Ok(())
}
...
fn rsa15a_mismatched_client_id_error() {
    ...
    match result {
        Err(err) => {
            assert_eq!(
                err.code,
                Some(crate::error::ErrorCode::InvalidClientID.code())
            );
        }
...
async fn rsa16a_token_details_from_request_token() -> Result<()> {
    ...
    let stored = client
        .auth()
        .token_details()
        .expect("should have stored token details");
...
fn rsa16b_token_details_from_token_string() {
    ...
    assert!(
        td.metadata.is_none(),
        "Token string should not have metadata"
    );
}
        ...
        if req.url.path().contains("revokeTokens") {
            if let Some(body) = &req.body {
                ...
                assert_eq!(
                    targets.len(),
                    1,
                    "Single specifier should be sent as array of 1"
                );
...
async fn rsa17c_mixed_revocation_result() -> Result<()> {
    ...
    let request = crate::rest::RevokeTokensRequest {
        targets: vec!["clientId:alice".to_string(), "clientId:unknown".to_string()],
        issued_before: None,
        allow_reauth_margin: None,
    };
    ...
    assert_eq!(result.len(), 2);
    assert!(
        result.results[0].error.is_none(),
...
async fn rsa5_ttl_null_when_unspecified() {
    ...
    assert!(
        req.ttl.is_none(),
        "ttl must be null when unspecified, got {:?}",
        req.ttl
    );
}
...
async fn rsa17f_both_options_together() -> Result<()> {
    ...
    let mock = MockHttpClient::with_handler(|_req| {
        MockResponse::json(
            200,
            &json!({"successCount": 1, "failureCount": 0, "results": [{"target": "clientId:user1"}]}),
        )
    });
    let client = mock_client_json(mock);
    client
