...
fn data_msgpack_round_trip_preserves_types() {
    ...
    let data = crate::rest::Data::Binary(serde_bytes::ByteBuf::from(vec![0x01, 0x02, 0x03, 0x04]));
    let packed = rmp_serde::to_vec_named(&data).unwrap();
    let unpacked: crate::rest::Data = rmp_serde::from_slice(&packed).unwrap();
    assert_eq!(
        unpacked,
...
async fn none_paginated_url_contains_channel() -> Result<()> {
    ...
    let reqs = get_mock(&client).captured_requests();
    assert!(
        reqs[0].url.path().contains("/channels/my-chan/"),
...
async fn none_paginated_presence_history_url() -> Result<()> {
    ...
    assert!(reqs[0]
        .url
        .path()
        .contains("/channels/pres-hist/presence/history"));
    Ok(())
}
...
fn none_error_with_cause_preserves_source() {
    ...
    assert_eq!(
        err.code,
        Some(crate::error::ErrorCode::ConnectionFailed.code())
    );
...
fn none_error_href_format() {
    ...
    assert_eq!(
        err.href.as_deref(),
        Some("https://help.ably.io/error/40142")
    );
}
...
async fn none_ably_agent_header_present() -> Result<()> {
    ...
    assert!(
        agent_str.contains("ably-rust"),
        "Ably-Agent should contain SDK identifier"
    );
    Ok(())
}
...
fn none_errorcode_channel_codes() {
    ...
    assert_eq!(
        ErrorCode::UnableToEnterPresenceChannelNoClientID.code(),
        91000
    );
}
