...
use crate::crypto::CipherParams;
    ...
    fn rtp2h2b_absent_members_deleted_on_end_sync() {
        ...
        assert_eq!(map.get("c1:alice").unwrap().action, Some(PresenceAction::Present));
    ...
    fn rtp17b_enter_after_enter_overwrites() {
        ...
        assert_eq!(map.values().len(), 1);
        assert_eq!(map.get("client-1").unwrap().action, Some(PresenceAction::Enter));
        assert_eq!(
    ...
    fn rtp17b_update_after_enter_overwrites() {
        ...
        assert_eq!(map.values().len(), 1);
        assert_eq!(map.get("client-1").unwrap().action, Some(PresenceAction::Update));
        assert_eq!(
            map.get("client-1").unwrap().data,
            crate::rest::Data::String("updated".to_string())
        );
    ...
    fn rtp2h2a_leave_during_sync_interaction_with_end_sync() {
        ...
        let leave_result = map.remove(&pm(PresenceAction::Leave, "bob", "c2", "c2:1:0", 200, None).member_key());
        let _ = leave_result;
        if let Some(stored) = map.get("c2:bob") {
            assert_eq!(stored.action, Some(PresenceAction::Absent));
        }
    ...
    async fn rtp6b_subscribe_filtered_single_action() {
        ...
        let _enter_id = channel
            .presence()
            .subscribe_action(crate::rest::PresenceAction::Enter, move |msg| { let _ = enter_tx.send(msg); });
        let (leave_tx, mut leave_rx) = tokio::sync::mpsc::unbounded_channel();
        let _leave_id = channel
            .presence()
            .subscribe_action(crate::rest::PresenceAction::Leave, move |msg| { let _ = leave_tx.send(msg); });
    ...
    async fn rtp7a_unsubscribe_specific_listener() {
        ...
        let presence = channel.presence();
        let (tx_a, mut rx_a) = tokio::sync::mpsc::unbounded_channel(); let id_a = presence.subscribe(move |msg| { let _ = tx_a.send(msg); });
        let (tx_b, mut rx_b) = tokio::sync::mpsc::unbounded_channel(); let _id_b = presence.subscribe(move |msg| { let _ = tx_b.send(msg); });
    ...
    async fn rtp7c_unsubscribe_all() {
        ...
        let presence = channel.presence();
        let (tx_a, mut rx_a) = tokio::sync::mpsc::unbounded_channel(); let _sub_a = presence.subscribe(move |msg| { let _ = tx_a.send(msg); });
        let (tx_b, mut rx_b) = tokio::sync::mpsc::unbounded_channel(); let _sub_b = presence.subscribe(move |msg| { let _ = tx_b.send(msg); });
    ...
    async fn rtp12c_presence_history_returns_paginated_result() -> Result<()> {
        ...
        let result: crate::http::PaginatedResult<crate::rest::PresenceMessage> = channel.presence().history().send().await?;
