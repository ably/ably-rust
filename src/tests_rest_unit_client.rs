...
async fn rsc13_request_timeout() -> Result<()> {
    ...
    assert_eq!(
        err.code,
        Some(crate::error::ErrorInfoCode::TimeoutError.code())
    );
...
async fn rsc10b_non_token_401_not_retried() -> Result<()> {
    ...
    assert_eq!(
        err.code,
        Some(crate::error::ErrorInfoCode::Unauthorized.code())
    );
...
async fn rsc15l4_cloudfront_error_triggers_fallback() -> Result<()> {
    ...
    mock.queue_response(
        MockResponse::json(
            403,
            &json!({"error": {"code": 40300, "message": "Forbidden"}}),
        )
...
async fn rsc15l4_non_cloudfront_4xx_no_fallback() -> Result<()> {
    ...
    assert_eq!(
        reqs.len(),
        1,
        "Non-CloudFront 403 should NOT trigger fallback"
    );
    Ok(())
}
...
async fn rec2c5_environment_sets_fallback_domains() -> Result<()> {
    ...
    let fallback_host = reqs[1].url.host_str().unwrap();
    assert!(
        expected_env_fallbacks.contains(&fallback_host),
...
async fn rec2c1_default_fallback_domains() -> Result<()> {
    ...
    let fallback_host = reqs[1].url.host_str().unwrap();
    assert!(
        expected_fallbacks.contains(&fallback_host),
...
async fn rsc6a_stats_no_params() -> Result<()> {
    ...
    assert!(!query.iter().any(|(k, _)| k == "end"));
    assert!(!query.iter().any(|(k, _)| k == "limit"));
    assert!(!query.iter().any(|(k, _)| k == "direction"));
...
async fn rsc19f_request_http_methods() -> Result<()> {
    for method in ["GET", "POST", "PUT", "PATCH", "DELETE"] {
        ...
        client.request(method, "/test").send().await?;
...
async fn rsc19b_request_uses_auth() -> Result<()> {
    ...
    client.request("GET", "/test").send().await?;
...
async fn bar2_mixed_success_failure_counts() -> Result<()> {
    ...
    let result = client
        .batch_presence(&["ch-1", "ch-2", "ch-3", "ch-4"])
        .await?;
...
async fn bgr2_success_result_members() -> Result<()> {
    ...
    assert_eq!(
        success.presence[0].connection_id.as_deref(),
        Some("conn-abc")
    );
...
async fn rsc19d_http_paginated_response_status_and_success() -> Result<()> {
    let mock =
        MockHttpClient::with_handler(|_req| MockResponse::json(200, &json!([{"key": "value"}])));
    let client = mock_client(mock);
...
async fn rsc15l_fallback_on_network_failure() -> Result<()> {
    ...
    assert!(
        ...
        "Should have retried on fallback host"
    );
    Ok(())
}
...
async fn rec1b4_fallback_on_5xx_error() -> Result<()> {
    ...
    mock.queue_response(MockResponse::json(
        500,
        &json!({"error": {"code": 50000, "statusCode": 500, "message": "Internal error"}}),
    ));
    mock.queue_response(MockResponse::json(200, &json!([1700000000000_i64])));
    let client = ClientOptions::new("appId.keyId:keySecret")
        .use_binary_protocol(false)
...
async fn rec2c3_dns_failure_triggers_fallback() -> Result<()> {
    ...
    let mock = MockHttpClient::with_handler(move |req| {
        ...
        urls_c
            .lock()
            .unwrap()
            .push(req.url.host_str().unwrap_or("").to_string());
        if n == 0 {
            MockResponse::network_error()
        } else {
            MockResponse::json(200, &json!([1700000000000_i64]))
    ...
    assert_ne!(
        captured_urls[0], captured_urls[1],
        "Second request should use a different (fallback) host"
    );
    Ok(())
}
...
async fn rec2c4_non_5xx_does_not_trigger_fallback() -> Result<()> {
    ...
    mock.queue_response(MockResponse::json(
        400,
        &json!({"error": {"code": 40000, "statusCode": 400, "message": "Bad request"}}),
    ));
    let client = ClientOptions::new("appId.keyId:keySecret")
        .use_binary_protocol(false)
        .rest_with_mock(mock)
...
async fn rec3a_fallback_retry_timeout() -> Result<()> {
    ...
    mock.queue_response(MockResponse::json(200, &json!([1700000000000_i64])));
    mock.queue_response(MockResponse::json(200, &json!([1700000000000_i64])));
    let mut opts = ClientOptions::new("appId.keyId:keySecret").use_binary_protocol(false);
...
async fn hp8_default_tls_port_in_url() -> Result<()> {
    ...
    assert!(
        url_str.starts_with("https://main.realtime.ably.net/"),
        "got: {}",
        url_str
    );
    Ok(())
...
async fn rsc7c_request_id_preserved_across_retries() -> Result<()> {
    ...
    assert_eq!(
        rid(0),
        rid(1),
        "request_id must be identical across retries"
    );
    Ok(())
}
...
async fn rsc7c_error_info_carries_request_id() -> Result<()> {
    let mock = MockHttpClient::with_handler(|_req| {
        MockResponse::json(
            404,
            &json!({"error": {"code": 40400, "statusCode": 404, "message": "nope"}}),
        )
    });
    let client = ClientOptions::new("appId.keyId:keySecret")
        .add_request_ids(true)
...
async fn rec1d_realtime_host_overrides_default_independently() -> Result<()> {
    ...
    let mut opts = ClientOptions::new("appId.keyId:keySecret").realtime_host("rt.example.com");
    opts.resolve_hosts();
...
async fn rsc10_token_renewal_on_401() -> Result<()> {
    ...
    assert!(
        token_reqs.len() >= 2,
        "Expected at least 2 token requests (initial + renewal)"
    );
    Ok(())
}
...
async fn rsc22_standard_headers() -> Result<()> {
    ...
    assert!(
        reqs[0].headers.iter().any(|(k, _)| k == "x-ably-version"),
...
async fn bpr1c_batch_publish_result_mixed() -> Result<()> {
    ...
    assert!(matches!(
        &results[0],
        crate::rest::BatchPublishResult::Success(_)
    ));
    assert!(matches!(
        &results[1],
        crate::rest::BatchPublishResult::Failure(_)
    ));
    Ok(())
}
...
async fn rsc8a_msgpack_content_type_depth() -> Result<()> {
    ...
    assert!(
        ct.contains("msgpack"),
        "Expected msgpack content-type, got: {}",
        ct
    );
    Ok(())
}
...
async fn rsc7c_request_id_format_depth() -> Result<()> {
    ...
    assert!(
        rid.len() >= 16,
        "request_id should be at least 16 chars, got {}",
        rid.len()
    );
    Ok(())
}
...
async fn rsc7c_request_id_unique_per_request() -> Result<()> {
    ...
    assert_ne!(
        rid(0),
        rid(1),
        "Each logical request gets a unique request_id"
    );
    Ok(())
}
...
async fn rsc22c_batch_publish_auth_header_depth() -> Result<()> {
    ...
    let mock = MockHttpClient::with_handler(|req| {
        assert!(
            ...
            "Batch publish should include auth header"
        );
        MockResponse::json(200, &json!([{"channel": "ch1", "messageId": "m1"}]))
    });
    let client = mock_client(mock);
...
async fn rsc19c_json_content_type_in_request_depth() -> Result<()> {
    ...
    assert!(
        ct.contains("json"),
        "Expected JSON Content-Type for JSON-mode request"
    );
    Ok(())
}
...
async fn bpr2_success_result_fields() -> Result<()> {
    ...
    assert_eq!(s.serials, Some(vec![Some("serial-1".to_string()), None]));
    Ok(())
}
...
async fn rsc15f_expired_fallback_not_resurrected() -> Result<()> {
    ...
    let to_fallback = reqs
        .iter()
        .filter(|r| r.url.host_str() == Some(fallback.as_str()))
        .count();
