...
async fn rtc8a_authorize_on_connected_sends_auth_message() {
    ...
    assert_eq!(
        auth_msgs[0].message.auth.as_ref().unwrap()["accessToken"].as_str(),
        Some("token-2")
    );
...
async fn rtc9_realtime_request_proxies_to_rest() -> Result<()> {
    ...
    let client = mock_client(mock);
    let resp = client.request("GET", "/custom/endpoint").send().await?;
...
async fn rtc8b_authorize_while_connecting() {
    ...
    assert!(
        ...
        "Should have made at least 2 connection attempts"
    );
...
async fn rtc8b1_authorize_while_connecting_on_failed() {
    ...
    assert!(
        result.is_err(),
        "authorize() should fail when connection goes to FAILED"
    );
}
...
async fn rtc5_close_channels_detached() {
    ...
    let attach_msg = sent.iter().find(|m| {
        m.message.action == action::ATTACH
            && m.message.channel.as_deref() == Some("test-close-channel")
    });
...
async fn rtc9_request_get() -> Result<()> {
    ...
    let client = mock_client(mock);
    let resp = client.request("GET", "/test/endpoint").send().await?;
    assert_eq!(resp.status_code(), 200);
    Ok(())
}
