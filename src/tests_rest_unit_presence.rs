...
use crate::crypto::CipherParams;
    ...
    async fn rsp3_get_with_404() -> Result<()> {
        ...
        let result = client.channels().get("nonexistent").presence().get().send().await;
    ...
    async fn rsp3a1b_default_limit_not_set_depth() -> Result<()> {
        ...
        let client = ClientOptions::new("appId.keyId:keySecret")
            ...
            .rest_with_mock(mock)
            .unwrap();
        client.channels().get("test").presence().get().send().await?;
        let reqs = get_mock(&client).captured_requests();
        let has_limit = reqs[0].url.query_pairs().any(|(k, _)| k == "limit");
        assert!(!has_limit, "Default presence get should not include limit param");
        Ok(())
    }
    ...
    async fn rsp4b1_history_start_param_depth() -> Result<()> {
        ...
        let client = ClientOptions::new("appId.keyId:keySecret")
            ...
            .rest_with_mock(mock)
            .unwrap();
        client.channels().get("test").presence().history()
            .start("1609459200000")
            .send()
            .await?;
        let reqs = get_mock(&client).captured_requests();
    ...
    async fn rsp_presence_message_with_string_data_depth() -> Result<()> {
        ...
        let res = client.channels().get("test").presence().get().send().await?;
        let items = res.items();
        assert_eq!(items[0].data, crate::rest::Data::String("plain-text-data".to_string()));
        Ok(())
    }
