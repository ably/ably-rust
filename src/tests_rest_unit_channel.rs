...
async fn rsl6b_unrecognized_encoding_preserved() -> Result<()> {
    ...
    assert_eq!(items[0].encoding, Some("custom-encoding".to_string()));
...
async fn rsl6b_cipher_step_without_cipher_leaves_residual() -> Result<()> {
    ...
    assert!(
        matches!(items[0].data, Data::Binary(_)),
        "base64 step was applied"
    );
    Ok(())
}
...
fn rse_crypto_fixtures_decode() {
    for file in ["crypto-data-128.json", "crypto-data-256.json"] {
        ...
        for (i, item) in fixture["items"].as_array().unwrap().iter().enumerate() {
            let mut msg: Message = serde_json::from_value(item["encrypted"].clone()).unwrap();
            msg.decode_with_cipher(Some(&cipher));

            let mut expected: Message = serde_json::from_value(item["encoded"].clone()).unwrap();
...
async fn rsl7_set_options_applies_cipher() -> Result<()> {
    ...
    ch.set_options(crate::rest::ChannelOptions {
        cipher: Some(cipher),
    });
...
async fn rsl1n_publish_returns_result_with_serials() -> Result<()> {
    ...
    let result = channel.publish().name("test").string("data").send().await?;
...
async fn rsl1i_message_exceeding_max_size_rejected() -> Result<()> {
    ...
    assert_eq!(
        reqs.len(),
        1,
        "Only the small message should have been sent"
    );
    Ok(())
}
...
async fn rsl1k2_idempotent_publish_message_id_format() -> Result<()> {
    ...
    let id = body["id"]
        .as_str()
        .expect("library-generated id must be present");
    let parts: Vec<&str> = id.split(':').collect();
    assert_eq!(
        parts.len(),
        2,
        "id format must be base:serial, got '{}'",
        id
    );
    assert!(
        parts[0].len() >= 12,
        ">= 9 bytes base64 encoded, got '{}'",
        parts[0]
    );
    assert!(
        parts[0]
...
async fn rsl1k2_serial_increments_batch() -> Result<()> {
    ...
    let arr = body
        .as_array()
        .expect("multi-message publish body is an array");
    let ids: Vec<(&str, &str)> = arr
        .iter()
        .map(|m| {
        ...
        .collect();
    assert!(
        ids.iter().all(|(base, _)| *base == ids[0].0),
        "same base for all"
    );
    assert_eq!(
        ids.iter().map(|(_, s)| *s).collect::<Vec<_>>(),
        vec!["0", "1", "2"]
    );
    Ok(())
}
...
async fn rsl1k_mixed_client_and_library_ids() -> Result<()> {
    ...
    let messages = vec![
        Message {
            ...
            data: Data::String("data1".into()),
            ..Default::default()
        },
        Message {
            name: Some("event2".into()),
            data: Data::String("data2".into()),
            ..Default::default()
        },
        Message {
            id: Some("client-id-2".into()),
            name: Some("event3".into()),
            data: Data::String("data3".into()),
            ..Default::default()
        },
    ];
    channel.publish().messages(messages).send().await?;
    ...
    let generated = arr[1]["id"]
        .as_str()
        .expect("library id for the middle message");
...
async fn rsl1k2_idempotent_disabled_no_auto_id() -> Result<()> {
    ...
    let has_id = body.get("id").is_some_and(|v| v.is_string());
    let array_has_id = body
        .as_array()
        .is_some_and(|a| a[0].get("id").is_some_and(|v| v.is_string()));
    assert!(
        !has_id && !array_has_id,
        "Should not auto-generate id when idempotent disabled"
    );
    Ok(())
}
...
async fn rsl4a_number_type_rejected() {
    ...
    assert_eq!(
        get_mock(&client).captured_requests().len(),
        0,
        "nothing sent"
    );
}
